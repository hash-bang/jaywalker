...
describe('CLI tests', ()=> {
	...
	it('should process simple stdin > stdout', done => {
		exec(`echo 'abc{"foo": "Foo!"}def' | app.js`, {cwd}, (err, res) => {
	...
	it('should process simple files (smallest chunk)', done => {
		exec(`app.js --smallest ${__dirname}/data/simple.txt`, {cwd}, (err, res) => {
			expect(err).to.be.not.ok;
	...
	it('should process simple files (largest chunk)', done => {
		exec(`app.js --largest ${__dirname}/data/simple.txt`, {cwd}, (err, res) => {
			expect(err).to.be.not.ok;
